...
public class SuffixTreeFromArray {
    ...
    private static class SuffixTreeNode {
        ...
        SuffixTreeNode(int id, int parent, Map<Character, Integer> children, int stringDepth, int edgeStart, int edgeEnd) {
            this.id = id;
            this.parent = parent;
    ...
    private Map<Integer, List<Edge>> suffixTreeFromSuffixArray(
            int[] suffixArray,
            int[] lcpArray,
            final String text) {
        List<SuffixTreeNode> nodeTree = new ArrayList<>();
        SuffixTreeNode root = new SuffixTreeNode(nodeTree.size(), -1, new TreeMap<>(), 0, -1, -1);
        nodeTree.add(root);
        int lcpPrev = 0;
        ...
        for (int i = 0; i < text.length(); i++) {
            ...
            if (currentNode.stringDepth == lcpPrev)
                currentNode = createNewLeaf(nodeTree, currentNode, text, suffix);
            else {
                int edgeStart = suffixArray[i - 1] + currentNode.stringDepth;
                int offset = lcpPrev - currentNode.stringDepth;
                SuffixTreeNode midNode = breakEdge(nodeTree, currentNode, text, edgeStart, offset);
                currentNode = createNewLeaf(nodeTree, midNode, text, suffix);
            }
            if (i < text.length() - 1)
                lcpPrev = lcpArray[i];
        }
        ...
        for (SuffixTreeNode current : nodeTree) {
            if (!current.children.isEmpty()) {
                List<Edge> neighbours = new ArrayList<>();
                for (Character c : current.children.keySet()) {
                    SuffixTreeNode child = nodeTree.get(current.children.get(c));
                    neighbours.add(new Edge(child.id, child.edgeStart, child.edgeEnd + 1));
                }
    ...
    private SuffixTreeNode createNewLeaf(List<SuffixTreeNode> nodeTree, SuffixTreeNode node, String text, int suffixIndex) {
        SuffixTreeNode leaf = new SuffixTreeNode(nodeTree.size(), node.id, new TreeMap<>(),
                text.length() - suffixIndex, suffixIndex + node.stringDepth, text.length() - 1);
        nodeTree.add(leaf);
        node.children.put(text.charAt(leaf.edgeStart), leaf.id);
        return leaf;
    }

    private SuffixTreeNode breakEdge(List<SuffixTreeNode> nodeTree, SuffixTreeNode node, String text, int start, int offset) {
        char startChar = text.charAt(start);
        char midChar = text.charAt(start + offset);
        SuffixTreeNode midNode = new SuffixTreeNode(nodeTree.size(), node.id, new TreeMap<>(), node.stringDepth + offset, start,
                start + offset - 1);
        nodeTree.add(midNode);
        nodeTree.get(node.children.get(startChar)).edgeStart += offset;
        midNode.children.put(midChar, node.children.get(startChar));
        nodeTree.get(node.children.get(startChar)).parent = midNode.id;
